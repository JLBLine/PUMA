        ...
        if len(repeated_cats)>1:
            ...
            if len(list(set([len(sset) for sset in sets])))==1 and len(sets[0])==len(repeated_cats):
                ...
                for set_ind,freqs in enumerate(set_freqs):
                    ...
                    new_set_ferrs.append(np.array(set_ferrs[set_ind])[indexes])
                    new_set_cats.append(np.array(set_cats[set_ind])[indexes])
                    new_set_names.append(np.array(set_names[set_ind])[indexes])
