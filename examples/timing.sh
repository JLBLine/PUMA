#!/bin/sh
START_TIME=$SECONDS
cross_match.py --separation=180 --make_table \
...
echo $PRINT_3
#-------------------------------------------------------------------------------------------------
calculate_bayes.py --primary_cat=mwacs --matched_cats=vlssr,mrc,sumss \
	--primary_freq=180 --matched_freqs=74,408,843 \
	--out_name=bayes_mw-v-m-s.txt
STRING_4="Time to calculate posterior probabilities: "
TIME_4=$SECONDS
ELAPSED_4=$(($TIME_4 - $TIME_3))
PRINT_4=$STRING_4$ELAPSED_4
echo $PRINT_4
# -------------------------------------------------------------------------------------------------
make_table.py --matched_cats=mwacs,vlssr,mrc,sumss \
	...
	--resolution=00:03:00 \
	--output_name=puma_mw-v-m-s --split=00:01:15 #--verbose #--figure_draw=4 #
STRING_5="Time to apply criteria and create final matched table: "
TIME_5=$SECONDS
